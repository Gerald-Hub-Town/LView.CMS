...
namespace LView.CMS.Services
{
    public class ManagerService : IManagerService
    {
        ...
        public async Task<BaseResult> ChangePasswordAsync(ChangePasswordModel model)
        {
            ...
            if (oldPwd == AESEncryptHelper.Encode(model.OldPassword, LViewCMSKeys.AesEncryptKeys))
            {
                var count = await _repository.ChangePasswordByIdAsync(model.Id, AESEncryptHelper.Encode(model.NewPassword.Trim(), LViewCMSKeys.AesEncryptKeys));
                if (count > 0)
                {
                    result.ResultCode = ResultCodeAddMsgKeys.CommonObjectSuccessCode;
                    result.ResultMsg = ResultCodeAddMsgKeys.CommonObjectSuccessMsg;
