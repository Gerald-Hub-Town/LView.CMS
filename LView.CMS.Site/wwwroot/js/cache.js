...
layui.use(['form', 'jquery', "layer"], function () {
    ...
    function showNotice() {
        layer.open({
            ...
            content: '<div style="padding:15px 20px; text-align:justify; line-height: 22px; text-indent:2em;border-bottom:1px solid #e2e2e2;">' +
                '<p>LViewCMS后台管理系统正式上线啦，界面采用贤心最新版layui及马哥的layuicms</p>' +
                '<p>喜欢的，快快star吧！</p></div>',
    ...
    function lockPage() {
        layer.open({
            ...
            content: '<div class="admin-header-lock" id="lock-box">' +
                '<div class="admin-header-lock-img"><img src="images/GeraldFace.png" class="userAvatar"/></div>' +
                '<div class="admin-header-lock-name" id="lockUserName">yilezhu</div>' +
                '<div class="input_btn">' +
                '<input type="password" class="admin-header-lock-input layui-input" autocomplete="off" placeholder="请输入密码解锁.." name="lockPwd" id="lockPwd" />' +
                '<button class="layui-btn" id="unlock">解锁</button>' +
                '</div>' +
                '<p>请输入“wangqi123.”，否则不会解锁成功哦！！！</p>' +
                '</div>',
            closeBtn: 0,
            shade: 0.9,
            success: function () {
    ...
    $("body").on("click", "#unlock", function () {
        if ($(this).siblings(".admin-header-lock-input").val() === '') {
            ...
            if ($(this).siblings(".admin-header-lock-input").val() === "wangqi123.") {
